...
def save_model(models_folder_path, encoder, decoder, optimizer, epoch, loss, batch_size, vocab):
    ...
    MODEL_PATH = MODELS_FOLDER / (model_name_formated("model",
                                  {"epoch": epoch}) + ".pth")
...
def generate_visualization_object(model_dir, dataset, predictions, targets):
    vis_obj = dict()
    ...
    with open(os.path.join(model_dir, "viz_obj.pkl"), "wb") as writer:
